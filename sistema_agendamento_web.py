...
if menu == "Cadastrar Artista":
    st.title("Cadastro de Artista")
    nome = st.text_input("Nome do Artista")
    servico = st.text_input("Serviço oferecido")
    preco = st.number_input("Preço do serviço (R$)", min_value=0.0, format="%.2f")
    disponibilidade = st.date_input("Disponibilidade de data")
    horario = st.time_input("Horário disponível")
...
# Página: Agendar
    ...
    if not st.session_state.artistas:
        st.warning("Nenhum artista cadastrado.")
    else:
        artista = st.selectbox("Escolha um artista", [a['nome'] for a in st.session_state.artistas])
...
# Página: Ver Agendamentos
    ...
    if not st.session_state.agendamentos:
        ...
        for i, ag in enumerate(st.session_state.agendamentos):
            st.write(f"**{i+1}. Artista:** {ag['artista']} | **Data:** {ag['data']} | **Hora:** {ag['hora']}")
